...
declare function toValidator(rule: Rule): ValidatorFn;
/** Wrap several sync core Rules as one Angular ValidatorFn (stops at first failure). */
declare function toValidators(rules: Rule[]): ValidatorFn;
/** Wrap a rule (sync or async, e.g. imageMaxDimensions) as an Angular AsyncValidatorFn. */
declare function toAsyncValidator(rule: Rule): AsyncValidatorFn;
/**
 ...
 */
declare function matchControls(fieldA: string, fieldB: string, message?: string): ValidatorFn;
/** Convenience: extract the first error message from an Angular control, if any. */
declare function getErrorMessage(control: AbstractControl | null): string | undefined;

export { type FieldErrors, type FieldValue, type Rule, type Schema, type ValidateSchemaResult, type ValidationResult, ageMinYears, alpha, alphaSpaces, alphanumeric, between, creditCard, custom, email, exactLength, fail, fileRequired, fileType, formatBytes, getErrorMessage, imageMaxDimensions, integer, isDate, isEmpty, matchControls, matchField, max, maxDate, maxFileCount, maxFileSize, maxLength, maxSelected, min, minDate, minFileCount, minFileSize, minLength, minSelected, mustBeChecked, negative, noWhitespace, notPlaceholder, numeric, numericString, ok, oneOf, oneOfStrings, pattern, phone, positive, required, selectedBetween, step, timeBetween, toAsyncValidator, toFileArray, toValidator, toValidators, url, validateField, validateFieldAsync, validateSchema, validateSchemaAsync, zipCode };
