...
__export(angular_exports, {
  ...
  toAsyncValidator: () => toAsyncValidator,
...
// src/angular/validators.ts
function toValidator(rule) {
  return (control) => {
    const result = validateField(control.value, [rule]);
    return result.valid ? null : { validation: result.message };
  };
}
function toValidators(rules) {
  return (control) => {
    const result = validateField(control.value, rules);
    return result.valid ? null : { validation: result.message };
  };
}
function toAsyncValidator(rule) {
  return async (control) => {
    const result = await validateFieldAsync(control.value, [rule]);
    return result.valid ? null : { validation: result.message };
  };
}
function matchControls(fieldA, fieldB, message = "Fields do not match") {
  return (group) => {
    var _a, _b;
    const a = (_a = group.get(fieldA)) == null ? void 0 : _a.value;
    const b = (_b = group.get(fieldB)) == null ? void 0 : _b.value;
    return a === b ? null : { mismatch: message };
  };
}
function getErrorMessage(control) {
  if (!control || !control.errors) return void 0;
